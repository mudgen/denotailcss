...
import shortenHex from "./shorten-hex.js";
import shortenHsl from "./shorten-hsl.js";
import shortenRgb from "./shorten-rgb.js";
import sortSelectors from "./sort-selectors.js";
import tidyRules from "./tidy-rules.js";
import tidyBlock from "./tidy-block.js";
import tidyAtRule from "./tidy-at-rule.js";
...
import formatPosition from "../../utils/format-position.js";
import split from "../../utils/split.js";
import { rules as serializeRules } from "../../writer/one-time.js";
...
import { DEFAULT as DEFAULT_ROUNDING_PRECISION } from "../../options/rounding-precision.js";
...
function optimizeBorderRadius(property) {
  ...
  if (
    values.length == 3 && values[1][1] == "/" && values[0][1] == values[2][1]
  ) {
    spliceAt = 1;
  } else if (
    values.length == 5 && values[2][1] == "/" && values[0][1] == values[3][1] &&
    values[1][1] == values[4][1]
  ) {
    spliceAt = 2;
  } else if (
    values.length == 7 && values[3][1] == "/" && values[0][1] == values[4][1] &&
    values[1][1] == values[5][1] && values[2][1] == values[6][1]
  ) {
    spliceAt = 3;
  } else if (
    values.length == 9 && values[4][1] == "/" && values[0][1] == values[5][1] &&
    values[1][1] == values[6][1] && values[2][1] == values[7][1] &&
    ...
    .replace(
      ...
      function (match, colorFunction, colorDef) {
        ...
        var applies = (colorFnLowercase == "hsl" && tokens.length == 3) ||
          ...
          (colorFnLowercase == "rgb" && tokens.length === 3 &&
            colorDef.indexOf("%") > 0) ||
          (colorFnLowercase == "rgba" && tokens.length == 4 &&
            colorDef.indexOf("%") > 0);
...
function optimizeMultipleZeros(property) {
  ...
  if (
    values.length == 4 && values[0][1] === "0" && values[1][1] === "0" &&
...
function optimizePixelLengths(_, value, compatibility) {
  ...
  return value.replace(WHOLE_PIXEL_VALUE, function (match, val) {
    ...
    if (
      compatibility.properties.shorterLengthUnits && compatibility.units.pt &&
      intVal * 3 % 4 === 0
    ) {
    ...
    if (
      compatibility.properties.shorterLengthUnits && compatibility.units.pc &&
      intVal % 16 === 0
    ) {
    ...
    if (
      compatibility.properties.shorterLengthUnits && compatibility.units.in &&
      intVal % 96 === 0
    ) {
  ...
  return value
    ...
    .replace(
      ...
      function (match, integerPart, fractionPart, unit) {
        ...
        return Math.round((integer + fraction) * multiplier) / multiplier +
          unit;
      },
    );
}
...
function optimizeUnits(name, value, unitsRegexp) {
  ...
  if (
    name == "flex" || name == "-ms-flex" || name == "-webkit-flex" ||
    name == "flex-basis" || name == "-webkit-flex-basis"
  ) {
    return value;
  }
  ...
  if (
    value.indexOf("%") > 0 &&
    (name == "height" || name == "max-height" || name == "width" ||
      name == "max-width")
  ) {
    return value;
  }
...
function optimizeWhitespace(name, value) {
  if (
    name.indexOf("filter") > -1 || value.indexOf(" ") == -1 ||
...
function removeQuotes(name, value) {
  if (
    name == "content" || name.indexOf("font-variation-settings") > -1 ||
    name.indexOf("font-feature-settings") > -1 || name == "grid" ||
    name.indexOf("grid-") > -1
  ) {
    return value;
  }
  ...
  for (var i = 0, l = _properties.length; i < l; i++) {
    ...
    if (!PROPERTY_NAME_PATTERN.test(name)) {
      ...
      context.warnings.push(
        "Invalid property name '" + name + "' at " +
          formatPosition(propertyToken[1][2][0]) + ". Ignoring.",
      );
    ...
    if (
      levelOptions.removeNegativePaddings && name.indexOf("padding") === 0 &&
      (isNegative(property.value[0]) || isNegative(property.value[1]) ||
        isNegative(property.value[2]) || isNegative(property.value[3]))
    ) {
      property.unused = true;
    }

    if (
      !options.compatibility.properties.ieFilters && isLegacyFilter(property)
    ) {
      property.unused = true;
    }
    ...
    for (var j = 0, m = property.value.length; j < m; j++) {
      ...
      if (type == Token.PROPERTY_BLOCK) {
        property.unused = true;
        context.warnings.push(
          "Invalid value token at " + formatPosition(value[0][1][2][0]) +
            ". Ignoring.",
        );
        break;
      }
      ...
      if (valueIsUrl && !context.validator.isUrl(value)) {
        ...
        context.warnings.push(
          "Broken URL '" + value + "' at " +
            formatPosition(property.value[j][2][0]) + ". Ignoring.",
        );
        break;
      }
    ...
    if (name == "background" && levelOptions.optimizeBackground) {
      ...
      name.indexOf("border") === 0 && name.indexOf("radius") > 0 &&
      levelOptions.optimizeBorderRadius
    ) {
      optimizeBorderRadius(property);
    } else if (
      name == "filter" && levelOptions.optimizeFilter &&
...
function cleanupCharsets(tokens) {
  ...
  for (var i = 0, l = tokens.length; i < l; i++) {
    ...
    if (hasCharset || token[1].indexOf(CHARSET_TOKEN) == -1) {
      ...
      tokens.splice(i, 1);
      tokens.unshift(
        [Token.AT_RULE, token[1].replace(CHARSET_REGEXP, CHARSET_TOKEN)],
      );
    }
  }
}
